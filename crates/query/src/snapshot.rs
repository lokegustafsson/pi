...
impl Snapshot {
    pub fn new(buf: &mut String, meminfo: &mut File, stat: &mut File) -> Self {
        meminfo.read_to_string(buf).unwrap();
        let mem_info = buf.parse().unwrap();
        buf.clear();
        meminfo.rewind().unwrap();

        stat.read_to_string(buf).unwrap();
        let cpus_stat = buf
            .lines()
